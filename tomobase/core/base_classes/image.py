...
class ImageAbstract(ContextModel, SignalModel, IOModel):
    """Base class for image models. This is to be used as a template for creating image models such as Sinogram, Volume etc. also useful for type hinting.

    Args:
        name (str): the name of the sample. This is the sample name
        data (array-like): the data of the image. This should be CUPY or NUMPY.
        pixel_size (float, optional): the size of a pixel. Defaults to 1.0.
        metadata (dict, optional): additional metadata for the image. Defaults to None.

    Properties:
        signal_labels (array-like): the labels for the signal dimension. This is used for labeling the signals in the image. This is only applicable if the image has a signal dimension.
        spatial_dims (list): the spatial dimensions of the image. This is used for identifying the spatial dimensions of the image. This is determined by checking for dimensions named 'x', 'y', or 'z'.
        non_spatial_dims (list): the non-spatial dimensions of the image. This is used for identifying the non-spatial dimensions of the image. This is determined by checking for dimensions that are not named 'x', 'y', or 'z'.
        process_name (str): a unique name for the current process. This is used for tracking the process history of the image. This is generated using a combination of a random slug and an iteration number.
        data (array-like): the data of the image. This is used for accessing and modifying the image data.
        pixel_size (float): the size of a pixel. This is used for scaling the coordinates of the spatial dimensions.
        metadata (dict): additional metadata for the image. This can be used to store any additional information about the image.

    Attributes:
        _allowed_dims (tuple): the allowed dimension names for the image. This is used for validating the dimensions of the image. The default allowed dimensions are 'signals', 'x', and 'y'.
    ...
    def transpose(self, *args):
        """Transposes the selected dims b

        Args:
            *args: the dimensions to transpose. This should be a list of dimension names in the order they should be transposed. The dimensions must be a subset of the existing dimensions of the image.

        Returns:
            ImageAbstract: a new image with the transposed dimensions.
        """
        for dim in args:
    ...
    @property
    def values(self):
        if hasattr(self.xr.values, "get"):
            values = self.xr.values.get()
        else:
            values = self.xr.values
    ...
    def coords_to_numpy(self):
        new_coords = {}
        for name, coord in self.xr.coords.items():
            data = coord.data

            if hasattr(data, "get"):
                data = data.get()

            new_coords[name] = (coord.dims, np.asarray(data))

        self.xr = self.xr.assign_coords(new_coords)
