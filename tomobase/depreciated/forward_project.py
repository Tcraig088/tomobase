...
def project_slices(volume:Volume, angles:Union[TiltSchemeCursor, np.ndarray], use_gpu:bool=True):
    ...
    proj_id = _create_projector(volume, angles)
    z, y, x = volume.xr.sizes['z'], volume.xr.sizes['y'], volume.xr.sizes['x']
    data = volume.xr.transpose("z", "y", "x").values
...
@registers.procedures.register(name='3D Project', category=registers.categories['Project'], use_numpy=True)
def project_3d(volume:Volume, angles:Union[TiltSchemeCursor, np.ndarray], use_gpu:bool=True):
    """Create a sinogram from a volume using forward projection. The GPU Context is overriden due to underlying astra gpu usage. 
    Args:
        volume (Volume): The input volume to be projected.
        angles (Union[Tuple[TiltSchemeAbstract, slice], np.ndarray]): The angles at which to project the volume. Can be a TiltSchemeAbstract with a slice of angles or a numpy array of angles.
    ...
    z, y, x = data.shape
    proj_id = _create_projector(data, angles, use_gpu)
    sino = np.empty((z, len(angles), max(x, y)))
    
    sino_id, sino = astra.creators.create_sino(data, proj_id)
    sinogram = Sinogram(volume.name, np.transpose(sino, (1,0,2)), angles, volume.pixel_size)  # ASTRA gives (z, n, d)
    astra.astra.delete(sino_id)
    astra.astra.delete(proj_id)
    return sinogram